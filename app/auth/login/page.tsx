...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (error) {
      ...
      setErrorMessage(errorMessages[error] || 'An error occurred during social login.')
      window.history.replaceState({}, '', '/auth/login')
    }
  }, [])

  // Demo Avatars using pravatar.cc
  const avatarUrls = [
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        notification.error({ message: 'Resend Failed', description: data.error || 'Failed to resend OTP code.' })
        setResendDisabled(false)
        setIsLoading(false)
        return
      }

      notification.success({ message: 'OTP Resent', description: 'A new verification code has been sent to your email.' })
      setOtp(['', '', '', '', '', ''])
      setTimer(300)
    } catch (error) {
      console.error('Resend OTP error:', error)
      notification.error({ message: 'Error', description: 'An error occurred while resending OTP.' })
      setResendDisabled(false)
    } finally {
      setIsLoading(false)
    }
  ...
  const renderFormContent = () => (
    <>
      ...
      <div className="flex flex-col items-center mb-8 relative z-10">
        ...
        {errorMessage && (
          <div className="mt-4 p-3 w-full rounded-lg bg-destructive/10 border border-destructive/20 text-destructive text-sm font-medium animate-in fade-in slide-in-from-top-1 text-center">
            {errorMessage}
          </div>
        )}
      </div>
        ...
        <form onSubmit={handleSubmitCredentials} className="space-y-5 relative z-10 animate-in fade-in duration-300">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <Input id="password" type={showPassword ? 'text' : 'password'} placeholder="Enter your password" value={password} onChange={(e) => setPassword(e.target.value)} className="pl-11 pr-11 h-12 bg-secondary/50 border-border focus:border-primary focus:ring-primary transition-all" required />
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Checkbox id="remember" checked={rememberMe} onCheckedChange={(checked) => setRememberMe(checked as boolean)} className="border-border data-[state=checked]:bg-primary data-[state=checked]:border-primary" />
          ...
          <Button type="submit" disabled={isLoading} className="w-full h-12 bg-primary hover:bg-primary/90 text-primary-foreground font-semibold rounded-xl transition-all duration-300 group">
            {isLoading ? (
              <div className="flex items-center gap-2"><div className="w-5 h-5 border-2 border-primary-foreground/30 border-t-primary-foreground rounded-full animate-spin" /><span>Signing in...</span></div>
            ) : (
              <div className="flex items-center gap-2"><span>Continue</span><ArrowRight className="w-5 h-5 transition-transform group-hover:translate-x-1" /></div>
            )}
          </Button>

          <div className="relative my-6">
            <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-border" /></div>
            <div className="relative flex justify-center"><span className="bg-card px-4 text-sm text-muted-foreground">Or continue with</span></div>
          </div>

          <div className="grid grid-cols-2 gap-3 relative z-10">
            <Button type="button" variant="outline" onClick={() => window.location.href = '/api/auth/callback/google'} disabled={isLoading} className="h-12 border-border hover:bg-secondary/50 hover:border-primary/30 transition-all bg-transparent">
            ...
            <Button type="button" variant="outline" onClick={() => window.location.href = '/api/auth/callback/github'} disabled={isLoading} className="h-12 border-border hover:bg-secondary/50 hover:border-primary/30 transition-all bg-transparent">
          ...
          <p className="text-center text-sm text-muted-foreground mt-6 relative z-10">
            Don't have an account? <Link href="/auth/signup" className="text-primary hover:text-primary/80 font-medium transition-colors">Create account</Link>
          </p>
        </form>
      )}
        ...
        <form onSubmit={handleVerifyOtp} className="space-y-6 relative z-10 animate-in fade-in duration-300">
          ...
          <Button type="submit" disabled={isLoading || otp.join('').length !== 6} className="w-full h-12 bg-primary hover:bg-primary/90 text-primary-foreground font-semibold rounded-xl transition-all duration-300 group">
            {isLoading ? (
              ...
              <div className="flex items-center gap-2"><span>Verify & Sign In</span><ArrowRight className="w-5 h-5 transition-transform group-hover:translate-x-1" /></div>
            )}
          </Button>

          <Button type="button" variant="outline" onClick={() => { setStep('credentials'); setOtp(['', '', '', '', '', '']); setTimer(300) }} className="w-full h-12 border-border hover:bg-secondary/50 transition-all bg-transparent">Back to Sign In</Button>
        </form>
      )}
      ...
      {step === 'success' && (
        <div className="space-y-6 relative z-10 animate-in fade-in duration-300 py-8 text-center">
          ...
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground">{twoFaEnabled ? 'Your login was verified successfully.' : 'You are now signed in.'}</p>
          ...
          <div className="flex-1 flex flex-col justify-center max-w-lg">
            <div className={`transition-all duration-700 ease-out ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/10 border border-primary/20 mb-6">
                ...
                <span className="text-xs font-semibold text-primary tracking-wide uppercase">Master Every Captcha</span>
              </div>

              <h2 className="text-4xl font-bold text-foreground leading-tight mb-4">
                Solve captchas <br /> <span className="text-primary">faster & smarter</span>
              </h2>
              <p className="text-muted-foreground text-base leading-relaxed mb-10">
                Join thousands of users who trust CaptchaⱮaster for lightning-fast, reliable captcha solving powered by cutting-edge AI technology.
              </p>
